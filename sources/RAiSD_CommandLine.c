...
void RSDHelp (FILE * fp)
{
	...
	fprintf(fp, "\t -I STRING\n");
	fprintf(fp, "\t[-arc SweepNet | FAST-NN | FASTER-NN | FASTER-NN-G]\n");
	fprintf(fp, "\t[-e INTEGER]\n");
	fprintf(fp, "\t[-cl4 label00=STRING label01=STRING label10=STRING label11=STRING]\n");
	fprintf(fp, "\t[-cl4_1x_label STRING]\n");
	fprintf(fp, "\t[-cl4_x1_label STRING]\n");
	fprintf(fp, "\t[-g INTEGER]\n");
	...
	fprintf(fp, "\n\t-- SLIDING WINDOW and MU STATISTIC \n\n");
	fprintf(fp, "\t-w\tProvides the window size (integer value). The default value is %d (empirically determined).\n", DEFAULT_WINDOW_SIZE);
...
SWP-SCN: Performs full scans for selective sweeps using a CNN generated by a previous MDL-GEN run. Requires: -G.\n\t\t\
         All generated images are stored in folder RAiSD_Grid.runID (automatically deleted, runID is the run ID that is set with -n).\n\t\t\
RSD-DEF: Standard RAiSD execution using only the mu-statistic (no CNN)\n");
	...
	fprintf(fp, "\t-w\tIn this mode (data generation), the window width provided via -w is used to generate SNP windows as CNN input data e.g., images (image width) (default: %d).\n", DEFAULT_WINDOW_SIZE_CNN);
...
void RSDCommandLine_init(RSDCommandLine_t * RSDCommandLine)
{
	...
	strncpy(RSDCommandLine->cl4_1x_label, "cl4_1x_label", STRING_SIZE);
	strncpy(RSDCommandLine->cl4_x1_label, "cl4_x1_label", STRING_SIZE); 
...
void RSDCommandLine_load(RSDCommandLine_t * RSDCommandLine, int argc, char ** argv)
{
	...
	for(i=1; i<argc; ++i)
	{
			...
			if (i!=argc-1 && argv[i+1][0]!='-')
			{
				...
				else
				{
					if(!strcmp(tstring2, "IMG-GEN")) 
					{
						...
						RSDCommandLine->windowSize = DEFAULT_WINDOW_SIZE_CNN;
					}
					else
					{
		...
		if(!strcmp(argv[i], "-cl4")) // class-pair associations for 2x2 model (FASTER-NN-G-2x2) 
		{ 
		...
		if(!strcmp(argv[i], "-cl4_1x_label"))
		{
			flagCheck (argv, i, flagVector, 47);
			
			if (i!=argc-1 && argv[i+1][0]!='-')
			{
				strcpy(RSDCommandLine->cl4_1x_label, argv[++i]);
			}			
			else
			{
				fprintf(stderr, "\nERROR: Missing argument after %s\n\n",argv[i]);
		...
		if(!strcmp(argv[i], "-cl4_x1_label"))
		{
			flagCheck (argv, i, flagVector, 48);
			
			if (i!=argc-1 && argv[i+1][0]!='-')
			{
				strcpy(RSDCommandLine->cl4_x1_label, argv[++i]);
			}			
			else
			{
				fprintf(stderr, "\nERROR: Missing argument after %s\n\n",argv[i]);
